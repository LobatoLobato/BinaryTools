...
[[maybe_unused]] BinaryWriter::BinaryWriter(const std::string& inputPath, bool truncate) {
  ...
  if (truncate)
    flags = std::ofstream::out | std::ofstream::binary |
            std::ofstream::trunc; //Clears existing contents of the file
  else
...
[[maybe_unused]] BinaryWriter::BinaryWriter(char* buffer, uint32_t sizeInBytes) {
...
[[maybe_unused]] void BinaryWriter::Flush() {
  stream_->flush();
}
...
[[maybe_unused]] void BinaryWriter::WriteUint16(uint16_t value) {
...
[[maybe_unused]] void BinaryWriter::WriteUint64(uint64_t value) {
  stream_->write(reinterpret_cast<const char*>(&value), 8);
}

[[maybe_unused]] void BinaryWriter::WriteInt8(int8_t value) {
  stream_->write(reinterpret_cast<const char*>(&value), 1);
}

[[maybe_unused]] void BinaryWriter::WriteInt16(int16_t value) {
  stream_->write(reinterpret_cast<const char*>(&value), 2);
}
...
[[maybe_unused]] void BinaryWriter::WriteInt64(int64_t value) {
  stream_->write(reinterpret_cast<const char*>(&value), 8);
}
...
[[maybe_unused]] void BinaryWriter::WriteNullTerminatedString(const std::string& value) {
  stream_->write(value.data(), std::streamsize(value.size()));
  WriteChar('\0');
}

[[maybe_unused]] void BinaryWriter::WriteFixedLengthString(const std::string& value) {
  stream_->write(value.data(), std::streamsize(value.size()));
}
...
[[maybe_unused]] void BinaryWriter::WriteDouble(double value) {
  stream_->write(reinterpret_cast<const char*>(&value), 8);
}
...
void BinaryWriter::SeekBeg(size_t absoluteOffset) {
  stream_->seekp(std::streamsize(absoluteOffset), std::ifstream::beg);
}

[[maybe_unused]] void BinaryWriter::SeekCur(size_t relativeOffset) {
  stream_->seekp(std::streamsize(relativeOffset), std::ifstream::cur);
}
...
void BinaryWriter::Skip(size_t bytesToSkip) {
  ...
  if (position + bytesToSkip > length) {
    ...
    stream_->seekp(std::streamsize(bytesAvailable), std::ifstream::cur);
    WriteNullBytes(bytesNeeded);
  } else
    stream_->seekp(std::streamsize(bytesToSkip), std::ifstream::cur);
}
...
[[maybe_unused]] size_t BinaryWriter::Align(size_t alignmentValue) {

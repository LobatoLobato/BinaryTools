...
struct Span {
public:
    [[maybe_unused]] explicit Span(const std::vector<T>& v) : ptr_(v.data()), size_(v.size()) {}
    ...
    //Returns reference to first member of the span
    [[maybe_unused]] const T &front() const { return *ptr_; }

    //Returns reference to last member of the span
    T &back() { return *(ptr_ + size_); }

    //Returns reference to last member of the span
    [[maybe_unused]] const T &back() const { return *(ptr_ + size_); }
